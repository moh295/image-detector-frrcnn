    ...
    for i in  range(number_of_frames):
        ...
        if i%args.skip==0 and images[i] is not None:
            ...
            if len(tensor_list)==args.batch:
                frame=model.predict(args.output, tensors=tensor_list,save_vid=True,count=count,images=image_batch,output_scale=args.output_scale,tracker=grasp_tracker,show_vid=True)
                count+=args.batch
                tensor_list = []
                image_batch=[]
    #check if there is still images in the list in case the previous loop break befor len(tensor_list)==args.batch
    if len(tensor_list):
        model.predict(args.output, tensors=tensor_list, save_vid=True, count=count, images=image_batch,output_scale=args.output_scale,tracker=grasp_tracker,show_vid=True)
    model.realse_vid()